...
- 웹 크롤링을 통해 다양한 뉴스 소스로부터 기사 데이터를 수집
    - `Selenium`: JavaScript 렌더링이 필요한 동적 웹 페이지에서 데이터를 추출 <br>
      ex) 특정 카테고리별 인기 기사
...
**4. Modeling/Inference (모델링 및 추론)**
- `PyTorch`, `Hugging Face Transformers`
- 전처리된 데이터를 바탕으로 사전 학습된 머신러닝 모델을 사용하여 텍스트로부터 추가적인 정보를 추론 <br>
ex) 기사 내용 기반 감성 분석, 핵심 내용 자동 요약, 주제 분류, 연관 기사 추천 등


**5. Deployment/Serving (배포 및 결과 제공)**
...
* **Speed Layer (스피드 레이어)**
    * 최신 데이터를 빠르게 처리하여 실시간 분석 결과를 제공 <br>
...
*   **Batch Layer (배치 레이어)**
    * 전체 데이터셋에 대해 정교하고 포괄적인 분석을 주기적으로 수행 <br>
...
*   **Serving Layer (서빙 레이어)**
    * 스피드 레이어의 실시간 뷰와 배치 레이어의 배치 뷰를 통합 <br>
...
news-pipeline/
├── .gitignore                 # .gitignore
├── assets/                    # README.md assets (Architecture, ETL diagram)
├── README.md                  # README.md
├── docker/                    # 모듈별 Dockerfile 정의
├── docker-compose.yml         # Docker Compose를 이용한 다중 컨테이너 애플리케이션 정의
├── infra/                     # 인프라 구성 관련 파일 (IaC)
...
└── src/                       # source code
    ├── api/                   # DBMS API Server
    ├── collectors/            # 데이터 수집기 
    ├── common/                # 공통 유틸리티 또는 설정
    ├── consumers/             # Kafka 컨슈머 모듈 (MongoDB 등)
