...
*   **Speed Layer (스피드 레이어):** 최신 데이터를 빠르게 처리하여 실시간 분석 결과를 제공 
    ex) RSS feed polling, Kafka Streaming
*   **Batch Layer (배치 레이어):** 전체 데이터셋에 대해 정교하고 포괄적인 분석을 주기적으로 수행 
    ex) 시간 단위 비동기 크롤링, `Airflow` 기반 ETL
*   **Serving Layer (서빙 레이어):** 스피드 레이어의 실시간 뷰와 배치 레이어의 배치 뷰를 통합
    ex) `MongoDB`, `Elasticsearch` 활용하여 사용자에게 완전한 데이터 뷰를 제공

**주요 이점:** 내결함성, 확장성, 다양한 데이터 처리 요구사항(실시간 및 배치) 동시 만족
...
├── assets/                    # 프로젝트 관련 이미지 에셋 (아키텍처, ETL 다이어그램)
