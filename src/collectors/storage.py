...
class MongoDBStorageManager:
    """Kafka raw 토픽의 뉴스 데이터를 MongoDB에 저장하는 관리자"""
    ...
    def __init__(self):
        ...
        self.consumer = KafkaConsumer(
            ...
            group_id='mongodb-storage-manager',
    ...
    def _create_indexes(self):
        ...
        self.db.news_articles.create_index([("url", 1)], unique=True)
    
    def _subscribe_to_raw_topics(self):
        """raw 토픽 구독"""
        self.consumer.subscribe(pattern='news\\..*\\.raw')
        logger.info("Kafka raw 토픽 구독 시작")
    
    def _get_main_category(self, sub_category: str) -> str:
        """서브 카테고리를 메인 카테고리로 매핑"""
        if sub_category in ['stock', 'realestate']:
            return 'economy'
        return sub_category
        ...
        try:
            for message in self.consumer:
                try:
                    ...
                    # 카테고리 매핑 (stock, realestate -> economy)
                    topic_parts = message.topic.split('.')
                    if len(topic_parts) >= 2:
                        news_data['category'] = self._get_main_category(topic_parts[1])
                    
                    # MongoDB에 저장
                    self._store_to_mongodb(news_data)
    ...
    def _store_to_mongodb(self, news_data: Dict[str, Any]):
        """MongoDB에 뉴스 데이터 저장"""
        try:
            self.db.news_articles.update_one(
                {"url": news_data["url"]},
                {"$set": news_data},
                upsert=True
            )
            logger.info(f"MongoDB 저장 완료: {news_data['title']}")
