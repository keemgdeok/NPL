...
class SummaryProcessor:
    ...
    def __init__(self,
        ...
        self.summarizer_extra_generate_kwargs = {
            ...
            if k not in [
                'default_model_name', 'input_max_length',
                'summary_min_length', 'summary_max_length',
                'num_beams', 'do_sample', 'min_text_length_for_summary_override'
            ]
        }
    ...
    def process_article(self, article_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            article_data (Dict[str, Any]): 처리할 기사 데이터.
                                           'content' 키에 요약할 텍스트가 있어야 합니다.

        Returns:
            Dict[str, Any]: 요약 정보가 포함된 기사 데이터.
        """
        content_to_summarize = article_data.get("content", "")
        
        if not content_to_summarize or len(content_to_summarize.strip()) < self.min_text_length_for_summary:
            logger.warning(
                f"요약할 content가 없거나 너무 짧습니다 (길이: {len(content_to_summarize.strip())} < {self.min_text_length_for_summary}). "
    ...
    def _process_buffered_batch(self, message_buffer: List[Dict[str, Any]]) -> None:
        ...
        for i, msg_bundle in enumerate(message_buffer):
            ...
            content_to_summarize = original_article.get("content", "")
            category_from_msg = msg_bundle.get('category', 'unknown_category_in_buffer')

            if not content_to_summarize or len(content_to_summarize.strip()) < self.min_text_length_for_summary:
                # 스킵 케이스: 바로 Kafka 메시지 빌드 및 전송
                logger.info(f"배치 내 메시지 스킵: URL {original_article.get('url')}, 키 'content' 길이 {len(content_to_summarize.strip())} (기준: {self.min_text_length_for_summary})")
                summary_res_skipped = {
                ...
                # 요약 대상: 리스트에 추가
                logger.info(f"배치 내 메시지 요약 대상 추가: URL {original_article.get('url')}, 키 'content' 길이 {len(content_to_summarize.strip())}")
